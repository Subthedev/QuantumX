                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-6">
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      <div className="text-xs text-slate-600 font-semibold uppercase mb-2">Total Signals</div>
                      <div className="text-3xl font-bold text-slate-800">{monthStats.totalSignals}</div>
                    </div>

                    <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                      <div className="text-xs text-emerald-600 font-semibold uppercase mb-2">Win Rate</div>
                      <div className="text-3xl font-bold text-emerald-700">{monthStats.winRate.toFixed(1)}%</div>
                      <div className="text-xs text-slate-600 mt-2">{monthStats.totalWins}W / {monthStats.totalLosses}L</div>
                    </div>

                    <div className={`p-4 rounded-lg border ${monthStats.totalReturn >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
                      <div className={`text-xs font-semibold uppercase mb-2 ${monthStats.totalReturn >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>Total Return</div>
                      <div className={`text-3xl font-bold flex items-center gap-2 ${monthStats.totalReturn >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                        {monthStats.totalReturn >= 0 ? <TrendingUp className="w-6 h-6" /> : <TrendingDown className="w-6 h-6" />}
                        {monthStats.totalReturn > 0 ? '+' : ''}{monthStats.totalReturn.toFixed(1)}%
                      </div>
                    </div>

                    <div className={`p-4 rounded-lg border ${monthStats.avgReturnPerTrade >= 0 ? 'bg-blue-50 border-blue-200' : 'bg-rose-50 border-rose-200'}`}>
                      <div className={`text-xs font-semibold uppercase mb-2 ${monthStats.avgReturnPerTrade >= 0 ? 'text-blue-600' : 'text-rose-600'}`}>Avg Return/Trade</div>
                      <div className={`text-3xl font-bold flex items-center gap-2 ${monthStats.avgReturnPerTrade >= 0 ? 'text-blue-700' : 'text-rose-700'}`}>
                        {monthStats.avgReturnPerTrade >= 0 ? <TrendingUp className="w-6 h-6" /> : <TrendingDown className="w-6 h-6" />}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-gradient-to-r from-emerald-50 to-emerald-100 rounded-lg border border-emerald-200">
                      <div className="text-xs text-emerald-600 font-semibold uppercase mb-2">Best Trade</div>
                    ...
                    <div className="p-4 bg-gradient-to-r from-rose-50 to-rose-100 rounded-lg border border-rose-200">
                      <div className="text-xs text-rose-600 font-semibold uppercase mb-2">Worst Trade</div>
