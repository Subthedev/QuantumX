...
const PriceChart: React.FC<PriceChartProps> = ({
  ...
  const loadChartData = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://api.coingecko.com/api/v3/coins/${coinId}/market_chart?vs_currency=usd&days=${config.days}&interval=${config.interval}`, {
      ...
      if (apiData.prices && apiData.prices.length > 0) {
        ...
        const formattedData = prices.map(([timestamp, price]: [number, number]) => ({
          ...
          price: price,
          volume: price * 1000000 * (Math.random() * 0.5 + 0.75),
          // Synthetic volume
          timestamp
        }));
        if (!isMountedRef.current) return;
  ...
  const calculateTechnicalIndicators = (data: any[]) => {
    ...
    const enriched = data.map((item, index) => {
      ...
      const ma7 = ma7Data.length > 0 ? ma7Data.reduce((sum, d) => sum + d.price, 0) / ma7Data.length : item.price;
      const ma25 = ma25Data.length > 0 ? ma25Data.reduce((sum, d) => sum + d.price, 0) / ma25Data.length : item.price;
      return {
        ...item,
        ma7,
        ma25
  ...
  return <Card className="p-4 bg-card/50 backdrop-blur-sm border-border/50">
      {/* Header */}
      ...
      {/* Chart */}
      {error ? <div className="w-full flex items-center justify-center bg-background/50 rounded-lg border border-dashed border-border/50" style={{
      height: `${height}px`
    }}>
          ...
          bottom: 5
              ...
              <defs>
                <linearGradient id={`color-${isPositive ? 'up' : 'down'}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={isPositive ? 'rgb(22, 219, 101)' : 'rgb(255, 95, 109)'} stopOpacity={0.4} />
                  <stop offset="95%" stopColor={isPositive ? 'rgb(22, 219, 101)' : 'rgb(255, 95, 109)'} stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255, 255, 255, 0.05)" vertical={false} />
              <XAxis dataKey="time" stroke="rgba(255, 255, 255, 0.3)" tick={{
            fill: 'rgba(255, 255, 255, 0.5)',
            fontSize: 11
          }} tickLine={{
            stroke: 'rgba(255, 255, 255, 0.1)'
          }} />
              <YAxis yAxisId="price" stroke="rgba(255, 255, 255, 0.3)" tick={{
            fill: 'rgba(255, 255, 255, 0.5)',
            fontSize: 11
          }} tickLine={{
            stroke: 'rgba(255, 255, 255, 0.1)'
          }} domain={['auto', 'auto']} tickFormatter={value => formatPrice(value)} />
              {showVolume && <YAxis yAxisId="volume" orientation="right" stroke="rgba(255, 255, 255, 0.2)" tick={{
            ...
            stroke: 'rgba(255, 255, 255, 0.1)'
              ...
              {supportResistance.support > 0 && <>
                  <ReferenceLine yAxisId="price" y={supportResistance.support} stroke="rgb(22, 219, 101)" strokeDasharray="5 5" strokeOpacity={0.6} label={{
              value: `Support: ${formatPrice(supportResistance.support)}`,
              fill: 'rgb(22, 219, 101)',
              fontSize: 10,
              position: 'left'
            }} />
                  <ReferenceLine yAxisId="price" y={supportResistance.resistance} stroke="rgb(255, 95, 109)" strokeDasharray="5 5" strokeOpacity={0.6} label={{
            ...
            return [value, name];
              ...
              {/* Volume Bars */}
              {showVolume && <Bar yAxisId="volume" dataKey="volume" fill="rgba(255, 255, 255, 0.15)" radius={[2, 2, 0, 0]} maxBarSize={20} />}
              
              {/* Moving Averages */}
              {showMA && <>
                  <Line yAxisId="price" type="monotone" dataKey="ma7" stroke="rgb(255, 193, 7)" strokeWidth={1.5} dot={false} strokeOpacity={0.8} animationDuration={300} />
                  <Line yAxisId="price" type="monotone" dataKey="ma25" stroke="rgb(156, 39, 176)" strokeWidth={1.5} dot={false} strokeOpacity={0.8} animationDuration={300} />
                </>}
              
              {/* Price Area */}
              <Area yAxisId="price" type="monotone" dataKey="price" stroke={isPositive ? 'rgb(22, 219, 101)' : 'rgb(255, 95, 109)'} strokeWidth={2.5} fill={`url(#color-${isPositive ? 'up' : 'down'})`} animationDuration={300} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>}
    </Card>;
