...
const CryptoTable: React.FC<CryptoTableProps> = ({
  ...
  if (loading) {
    return <div className="space-y-0">
        {Array.from({
        length: 10
      }).map((_, i) => <div key={i} className="flex items-center p-4 border-b">
  ...
  return <>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border/50">
              {cryptos.map(crypto => <tr key={crypto.id} className="hover:bg-muted/20 transition-colors group">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-3">
                      <img src={crypto.image} alt={crypto.name} className="w-10 h-10 rounded-full" loading="lazy" />
                  ...
                  <td className="p-3 text-right">
                    <div className={cn("flex items-center justify-end gap-1 text-sm font-semibold", crypto.price_change_percentage_24h >= 0 ? "text-green-600" : "text-red-600")}>
                  ...
                  <td className="p-3 text-right hidden lg:table-cell">
                    <div className={cn("flex items-center justify-end gap-1 text-sm font-semibold", (crypto.price_change_percentage_7d_in_currency || 0) >= 0 ? "text-green-600" : "text-red-600")}>
                      {(crypto.price_change_percentage_7d_in_currency || 0) >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                      <span>
                  ...
                  <td className="p-3">
                    <div className="flex items-center justify-center gap-1">
                      <Button size="sm" variant="ghost" onClick={() => handleViewDetails(crypto)} className="h-8 w-8 p-0 hover:bg-muted" title="View Details">
                        <Eye className="w-4 h-4" />
                      </Button>
                      <Button size="sm" onClick={() => handleGenerateAIAnalysis(crypto)} className="h-8 px-3 bg-primary hover:bg-primary/90 text-primary-foreground gap-1.5">
      ...
      <Dialog open={showAIAnalysis} onOpenChange={setShowAIAnalysis}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedCoinForAnalysis && <CryptoReport coin={selectedCoinForAnalysis.symbol.toUpperCase()} icon={<img src={selectedCoinForAnalysis.image} alt={selectedCoinForAnalysis.name} className="w-8 h-8 rounded-full" />} name={selectedCoinForAnalysis.name} />}
        </DialogContent>
      </Dialog>
    </>;
};
const SparklineChart: React.FC<{
  ...
  return <svg width={width} height={height} className="mx-auto">
      <polyline points={points} fill="none" stroke={isPositive ? 'hsl(var(--success))' : 'hsl(var(--destructive))'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>;
};
