...
export default function FeedbackModal({
  ...
  const questions = [{
    ...
    options: [{
      value: "very_satisfied_daily",
      label: "Very satisfied, use daily for most trading decisions"
    }, {
      value: "satisfied_weekly",
      label: "Satisfied, use 2-3 times per week for key trades"
    }, {
      value: "somewhat_satisfied_occasional",
      label: "Somewhat satisfied, use occasionally when uncertain"
    }, {
      value: "not_satisfied_rarely",
      label: "Not very satisfied, rarely use despite having access"
    }]
  }, {
    id: "question_2",
    ...
    options: [{
      ...
      label: "Extremely likely (9-10) - I actively recommend it"
    }, {
      value: "very_likely",
      label: "Very likely (7-8) - I would recommend with minor reservations"
    }, {
      value: "somewhat_likely",
      label: "Somewhat likely (5-6) - I might recommend in specific situations"
    }, {
      value: "not_likely",
      label: "Not likely (0-4) - I would not recommend in its current state"
    }]
  }];
  const handleNext = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const {
        error: creditError
      } = await supabase.rpc('grant_feedback_credits', {
        _user_id: user.id,
        _credits: 5
      });
      ...
      <DialogContent className="sm:max-w-[600px] !bg-white dark:!bg-gray-900 !border-2 !border-gray-300 dark:!border-gray-700 !shadow-2xl">
        ...
        <div className="space-y-6 py-4">
          <div className="space-y-3">
            ...
            {currentQuestion.type === "text" ? <Textarea value={answers[currentQuestion.id as keyof typeof answers]} onChange={e => setAnswers({
            ...answers,
            [currentQuestion.id]: e.target.value
          })} placeholder="Be honest and specific - your genuine feedback shapes our AI improvements..." className="min-h-[100px] bg-background/50 border-primary/20 focus:border-primary/40" /> : <RadioGroup value={answers[currentQuestion.id as keyof typeof answers]} onValueChange={value => setAnswers({
            ...answers,
            [currentQuestion.id]: value
          })} className="space-y-3">
                {currentQuestion.options?.map(option => <div key={option.value} className="flex items-center space-x-3 p-3 rounded-lg border border-primary/10 hover:bg-primary/5 transition-colors cursor-pointer">
          ...
          <div className="flex justify-between gap-3 pt-4">
            <Button variant="outline" onClick={handlePrevious} disabled={currentStep === 0 || loading} className="flex-1">
              Previous
            </Button>
