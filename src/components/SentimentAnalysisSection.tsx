...
export const SentimentAnalysisSection: React.FC<SentimentAnalysisProps> = ({
  analysis
}) => {
  ...
  return <Card className="border-2 border-orange-500/20 shadow-xl bg-gradient-to-br from-background to-orange-500/5">
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          ...
          {analysis?.overall && <Badge variant="outline" className={`px-3 py-1 ${getSentimentColor(sentimentScore)}`}>
              {analysis.overall}
            </Badge>}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        {analysis ? <>
            ...
            {/* Sentiment Factors */}
            {analysis.factors && analysis.factors.length > 0 && <div className="space-y-2">
                <div className="text-sm font-medium text-muted-foreground">Contributing Factors</div>
                <div className="space-y-1">
                  {analysis.factors.map((factor: string, i: number) => <div key={i} className="flex items-start gap-2 p-2 bg-muted/30 rounded">
                      <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${factor.toLowerCase().includes('bullish') ? 'bg-green-500' : factor.toLowerCase().includes('bearish') ? 'bg-red-500' : 'bg-yellow-500'}`} />
            ...
            <div className="grid grid-cols-2 gap-3">
              {analysis.market_sentiment && <div className="p-3 bg-blue-500/10 rounded-lg border border-blue-500/20">
              ...
              {analysis.social_metrics && <div className="p-3 bg-purple-500/10 rounded-lg border border-purple-500/20">
                  <div className="flex items-center gap-2 mb-1">
            ...
            {/* Fear & Greed Analysis */}
            {analysis.fear_greed_analysis && <div className="p-4 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 rounded-lg border border-yellow-500/20">
            ...
            {/* Options Flow */}
            {analysis.options_flow && <div className="p-4 bg-gradient-to-r from-indigo-500/10 to-blue-500/10 rounded-lg border border-indigo-500/20">
                <div className="flex items-center gap-2 mb-2">
            ...
            {analysis.contrarian_indicators && <div className="p-4 bg-gradient-to-r from-pink-500/10 to-rose-500/10 rounded-lg border border-pink-500/20">
                <div className="flex items-center gap-2 mb-2">
