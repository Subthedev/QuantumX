/**
 * IGX BETA MODEL
 * Strategy matching and signal generation engine
 *
 * RESPONSIBILITIES:
...
export class IGXBetaModel {
  ...
  private initializeStrategies() {
    ...
    console.log('[IGX Beta] ✅ Initialized 10 strategies');
  }
  ...
  start() {
    if (this.isRunning) {
      console.log('[IGX Beta] Already running');
      return;
    }
  ...
  private async handleTickerUpdate(event: CustomEvent) {
    ...
    if (!ticker || !this.isRunning) {
      console.log('[IGX Beta] ⚠️ Ticker ignored:', ticker ? 'System not running' : 'Empty ticker');
      return;
    }

    this.stats.tickersAnalyzed++;
    console.log(`[IGX Beta] 📥 Received ticker #${this.stats.tickersAnalyzed}: ${ticker.symbol} @ $${ticker.price.toFixed(2)}`);
    ...
    if (!previousTicker) {
      console.log(`[IGX Beta] ⏳ Waiting for second ticker for ${ticker.symbol} (need 2 for pattern detection)`);
    ...
    if (strongPatterns.length === 0) {
      console.log(`[IGX Beta] ⚠️ Patterns too weak for ${ticker.symbol} (${patterns.length} detected, 0 strong enough)`);
      return; // Patterns too weak
    }

    console.log(`[IGX Beta] ✅ Strong patterns found for ${ticker.symbol}: ${strongPatterns.length}/${patterns.length}`);
    ...
    if (signal) {
      ...
      console.log(`[IGX Beta] ❌ Signal rejected after strategy analysis for ${ticker.symbol}`);
    }
  }
  ...
  private async detectPatterns(
    ...
    try {
      ...
      if (result.patterns.length > 0) {
        console.log(`[IGX Beta] 🔍 Patterns detected for ${current.symbol}:`,
      ...
      console.error('[IGX Beta] Pattern detection error:', error);
      return [];
    }
  }
  ...
  /**
   * Get threshold from Alpha model (would be injected)
   */
  private getAlphaThreshold(): number {
    // This would come from Alpha model
  ...
  private async analyzeWithStrategies(
    ...
    const strategyResults = await Promise.all(
      Array.from(this.strategies.entries()).map(async ([name, strategy]) => {
        try {
          ...
          console.error(`[IGX Beta] Strategy ${name} error:`, error);
          return null;
        }
      })
    );
    ...
    if (validResults.length === 0) {
      console.log('[IGX Beta] ❌ All strategies rejected');
      return null;
    }
    ...
    console.log(`[IGX Beta] 📊 Strategy votes: ${consensus.long}L / ${consensus.short}S / ${consensus.neutral}N`);

    // Select best signal using ML weights
    const bestSignal = this.selectBestSignal(validResults, consensus);

    if (!bestSignal) {
      console.log('[IGX Beta] ❌ No consensus reached');
      return null;
    }
  ...
  private async enrichTickerData(ticker: IGXTicker): Promise<any> {
    try {
      ...
      console.error('[IGX Beta] Enrichment error:', error);
  ...
  private selectBestSignal(results: any[], consensus: any): any {
    ...
    console.log(`[IGX Beta] 🏆 Best strategy: ${best.name} (score: ${best.score.toFixed(2)})`);
  ...
  private createSignal(
    ...
    console.log(`\n[IGX Beta] ✅ 🚀 SIGNAL GENERATED: ${signal.symbol} ${signal.direction}`);
  ...
  updateStrategyPerformance(
    ...
    console.log(`[IGX Beta] Strategy ${strategyName} updated: WR=${(performance.winRate * 100).toFixed(1)}% Weight=${performance.weight.toFixed(3)}`);
  }
  ...
  stop() {
    ...
    console.log('[IGX Beta] ✅ Stopped successfully');
  }
}

// Singleton instance
export const igxBetaModel = new IGXBetaModel();