...
class GlobalHubService extends SimpleEventEmitter {
  ...
  // ✅ LOGO CACHE - 2-minute cache like Dashboard for optimal performance
  ...
  public async start() {
    ...
    if (oldActiveCount > 0 || oldHistoryCount > 0) {
      console.log(`[GlobalHub] 🧹 Cleared old localStorage signals: ${oldActiveCount} active, ${oldHistoryCount} history`);
      console.log(`[GlobalHub] ✅ Tier-based system active - signals now in user_signals table only`);
      this.saveSignals(); // Save cleared state
    }
    ...
    try {
      ...
      scheduledSignalDropper.onDrop((signal, tier) => {
        console.log('\n' + '🎯'.repeat(40));
        console.log(`[GlobalHub] 🎯🎯🎯 ONDROP CALLBACK TRIGGERED! 🎯🎯🎯`);
        console.log(`[GlobalHub] Tier: ${tier}`);
        console.log(`[GlobalHub] Signal: ${signal.symbol} ${signal.direction}`);
        console.log(`[GlobalHub] About to call publishApprovedSignal...`);
        console.log('🎯'.repeat(40) + '\n');

        this.publishApprovedSignal(signal).then(() => {
          console.log('\n' + '🎊'.repeat(40));
          console.log('[GlobalHub] 🎊 publishApprovedSignal COMPLETED SUCCESSFULLY!');
          console.log('🎊'.repeat(40) + '\n');
        }).catch(err => {
          console.error('\n' + '❌'.repeat(40));
          console.error('[GlobalHub] ❌ Failed to publish scheduled signal:', err);
  ...
  private async publishApprovedSignal(displaySignal: HubSignal): Promise<void> {
    try {
      ...
      // ✅ TRACK: Signal published and added to active array
      this.state.metrics.publishingAddedToArray = (this.state.metrics.publishingAddedToArray || 0) + 1;

      console.log(`✅ Signal published to DATABASE AND activeSignals array`);
      console.log(`📊 activeSignals updated: ${this.state.activeSignals.length} active signals`);
      console.log(`📢 Signal saved to: intelligence_signals + user_signals + activeSignals`);
      ...
      console.log(`\n${'🔵'.repeat(40)}`);
      console.log(`[GlobalHub] 🔵 ABOUT TO CALL saveSignalToDatabase...`);
      console.log('🔵'.repeat(40));

      try {
        await this.saveSignalToDatabase(displaySignal);
        console.log(`\n${'🟢'.repeat(40)}`);
        console.log(`[GlobalHub] 🟢 saveSignalToDatabase COMPLETED SUCCESSFULLY`);
        console.log('🟢'.repeat(40) + '\n');
      } catch (dbError) {
        console.error(`\n${'🔴'.repeat(40)}`);
        console.error(`[GlobalHub] 🔴 saveSignalToDatabase FAILED!`);
        console.error(`[GlobalHub] Error:`, dbError);
        console.error('🔴'.repeat(40) + '\n');
        // Continue anyway - we still want to distribute to users
      }
      ...
      console.log(`\n${'💜'.repeat(40)}`);
      console.log(`[GlobalHub] 💜 ABOUT TO CALL distributeToUserSignals...`);
      console.log('💜'.repeat(40) + '\n');

      await this.distributeToUserSignals(displaySignal);

      console.log(`\n${'💚'.repeat(40)}`);
      console.log(`[GlobalHub] 💚 distributeToUserSignals COMPLETED!`);
  ...
  private async processGammaFilteredSignal(decision: GammaFilterDecision): Promise<void> {
    ...
    try {
      ...
      if (filteredSignal.passed) {
        ...
        // 🎯 EXACT SAME LOGO SYSTEM AS DASHBOARD - Perfect Match!
        // Uses cryptoDataService.getTopCryptos(100) with 2-minute cache just like Dashboard/CryptoTable
        let image = '';
        let coinGeckoId = '';

        try {
          // ✅ SMART CACHING: Only fetch if cache expired (2 minutes like Dashboard)
          const now = Date.now();
          ...
          if (crypto) {
            ...
            console.log(`[GlobalHub] ✅ Got PERFECT logo (Dashboard method) for ${signalInput.symbol}`);
            console.log(`[GlobalHub] 🖼️  IMAGE URL: "${image}"`);
            console.log(`[GlobalHub] 🆔 CoinGecko ID: "${coinGeckoId}"`);
          } else {
            console.log(`[GlobalHub] ⚠️ ${signalInput.symbol} not found in top 100 (cleaned: ${cleanSymbol})`);
          }
        } catch (error) {
          console.error(`[GlobalHub] ❌ Error fetching logos from CoinGecko:`, error);
        }
        ...
        console.log(`   🖼️  Image URL in displaySignal: "${displaySignal.image}"`);
  ...
  private async distributeToUserSignals(signal: HubSignal): Promise<void> {
    try {
      ...
      // ✅ TESTING MODE: Get ALL authenticated users (will filter by tier later)
      // In production, this would only get MAX tier users
      const { data: { user: currentUser }, error: userError } = await supabase.auth.getUser();

      if (userError || !currentUser) {
        console.error('[GlobalHub] ❌ Error getting current user:', userError);
        console.log('[GlobalHub] ⚠️ No authenticated user - signal saved to intelligence_signals only');
        console.log(`${'─'.repeat(80)}\n`);
        return;
      }
      ...
      if (subError) {
        console.error('[GlobalHub] ❌ Error fetching subscription:', subError);
      }
      ...
      // Use current user for distribution (testing mode - distribute to whoever is logged in)
      const maxUsers = [{ user_id: currentUser.id }];
      console.log(`\n👥 Distributing to current user (${userTier} tier)`);
      ...
      for (const user of maxUsers) {
        ...
        // In production, uncomment the quota check below
        /*
        ...
        if (!canReceive) {
          quotaExceededCount++;
          console.log(`[GlobalHub] ⚠️ User quota exceeded, skipping`);
          continue;
        }
        */
        ...
        // 🚀 INSTANT UI UPDATE: Emit event BEFORE database save for 0-lag experience
        // The UI will catch this and display the signal immediately
        const instantSignal = {
          id: signal.id,
          symbol: signal.symbol,
          signal_type: signal.direction,
        ...
        console.log(`[GlobalHub] ⚡ INSTANT signal event dispatched for ${signal.symbol}`);

        // Insert into user_signals
        console.log(`[GlobalHub] 💾 Saving to database - signal.image: "${signal.image}"`);
        const { error: insertError } = await supabase
          .from('user_signals')
          .insert({
            user_id: user.user_id,
            signal_id: signal.id,
            tier: userTier, // Use actual user tier
            symbol: signal.symbol,
            signal_type: signal.direction,
            confidence: signal.confidence || 0,
            quality_score: signal.qualityScore || 0,
        ...
        if (insertError) {
          ...
          if (insertError.code === '23505') {
            ...
            console.error(`\n${'❌'.repeat(40)}`);
            console.error(`[GlobalHub] ❌ CRITICAL: Error distributing to user ${user.user_id}`);
            console.error(`[GlobalHub] Error code: ${insertError.code}`);
            console.error(`[GlobalHub] Error message: ${insertError.message}`);
            console.error(`[GlobalHub] Error details:`, insertError);
            console.error(`[GlobalHub] Signal data:`);
            console.error(`  - Symbol: ${signal.symbol}`);
            console.error(`  - Direction: ${signal.direction}`);
            console.error(`  - Confidence: ${signal.confidence}`);
            console.error(`  - User ID: ${user.user_id}`);
            console.error(`  - Tier: ${userTier}`);
            console.error(`${'❌'.repeat(40)}\n`);
          }
          continue;
        }
        ...
        console.log(`\n${'✅'.repeat(40)}`);
        console.log(`[GlobalHub] ✅✅✅ SIGNAL INSERTED INTO user_signals TABLE! ✅✅✅`);
        console.log(`[GlobalHub] User: ${currentUser.email}`);
        console.log(`[GlobalHub] Signal: ${signal.symbol} ${signal.direction}`);
        console.log(`[GlobalHub] Confidence: ${signal.confidence}%`);
        console.log(`[GlobalHub] Expiry: ${expiresAt.toLocaleString()}`);
        console.log(`${'✅'.repeat(40)}\n`);

        // Increment quota using RPC function (optional in testing mode)
        ...
        if (incrementError) {
          console.warn(`[GlobalHub] ⚠️  Error incrementing quota (non-critical):`, incrementError);
        } else {
          console.log(`[GlobalHub] ✅ Quota incremented successfully`);
        }
    ...
    try {
      ...
      if (activeError) {
        ...
        for (const dbSignal of activeSignals) {
          ...
          // }
          console.log(`[GlobalHub] ℹ️ Signal ${hubSignal.id} found in database but NOT loading to localStorage (tier-based system active)`);
  ...
  public injectSignal(signal: Partial<HubSignal>) {
    ...
    this.state.metrics.totalSignals++;
    console.log('[GlobalHub] ℹ️ Injected signal NOT added to localStorage (tier-based system active)');
...
// ❌ CLIENT-SIDE GENERATION DISABLED: Signals now generated SERVER-SIDE 24/7
// The signal-generator Edge Function runs on Supabase servers every 30 seconds
// Frontend is now a PASSIVE RECEIVER ONLY - zero lag on refresh!
//
// To manually start for testing/diagnostics: window.globalHubService.start()
//
// if (typeof window !== 'undefined') {
...
//           try {
//             await globalHubService.start();
//             console.log('[GlobalHub] ✅ Retry auto-start successful');
//           } catch (retryError) {
//             console.error('[GlobalHub] ❌ Retry also failed:', retryError);
//           }
//         }, 2000);
//       }
//     }
...
console.log('[GlobalHub] ✅ Signals generated 24/7 by Supabase Edge Functions');
console.log('[GlobalHub] ✅ Frontend is PASSIVE RECEIVER - zero refresh lag!');
console.log('[GlobalHub] 📡 Listening for signals via Supabase Real-time...');
